...
const DocumentUploadDialog: React.FC<DocumentUploadDialogProps> = ({
	...
	const handleConfirm = () => {
		if (documents.some((doc) => doc.typeId.trim() === "")) {
			toast.warning(t("uploadDocuments.typeRequired"));
			return;
		}
		onConfirm(documents);
		setDocuments([]);
		...
		<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
			<div className="bg-white dark:bg-gray-900 rounded-md shadow-lg w-full max-w-2xl max-h-[90vh] overflow-hidden border border-gray-300 dark:border-gray-700">
				...
				<div className="p-6 overflow-y-auto max-h-[calc(90vh-140px)]">
					...
					>
						...
						<p className="text-base font-medium text-gray-900 dark:text-white mb-1">
							{t("uploadDocuments.dragAndDrop")}
						</p>
						<p className="text-sm text-gray-500 dark:text-gray-400 mb-3">
							{t("uploadDocuments.orClick")}
						</p>
						<p className="text-xs text-gray-400 dark:text-gray-500">
							{t("uploadDocuments.supportedFormats")}
						</p>
						<input
							type="file"
							multiple
											...
											<div className="flex-1 min-w-0">
												...
												<div className="space-y-2">
													<label className="block text-xs font-medium text-gray-700 dark:text-gray-300">
														{t("uploadDocuments.documentType")}{" "}
													...
													>
														<option value="" disabled>
															{loading
																? t("uploadDocuments.loadingTypes")
																: t("uploadDocuments.selectDocumentType")}
														</option>
					...
					<div className="space-y-4">
						...
						{!addingType ? (
							...
							>
								...
								{t("uploadDocuments.addNewDocumentType")}
							</button>
						) : (
							<div className="flex items-center gap-2">
								...
								>
									{loading
										? t("uploadDocuments.adding")
										: t("uploadDocuments.add")}
								</button>
								<button
									type="button"
									onClick={() => {
				...
				<div className="px-6 py-4 bg-gray-50 dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 flex justify-end gap-3">
					...
					>
						{t("common.confirm", { count: documents.length })}
					</button>
				</div>
			</div>
		</div>
